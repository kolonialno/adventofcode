...
pub fn run() {
    ...
    let positions = positions
        ...
        .collect::<Vec<isize>>();
    let min_fuel = find_min_fuel(&positions, |i, j| (i - j).abs());
    println!("Part 1: {}", min_fuel);
    let min_fuel = find_min_fuel(&positions, |i, j| ((i - j).abs() * ((j - i).abs() + 1)) / 2);
    println!("Part 2: {}", min_fuel);
}

fn find_min_fuel(positions: &Vec<isize>, diff_function: fn(i: isize, j: isize) -> isize) -> isize {
