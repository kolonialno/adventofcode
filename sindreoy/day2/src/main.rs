fn main() {
    ...
    let score_vec = calc_score_all_moves(opponent_moves, best_responses);
...
fn calc_score_all_moves(opponent_moves: Vec<&str>, best_responses: Vec<&str>) -> Vec<i32> {
    assert_eq!(opponent_moves.len(), best_responses.len());
    let mut score_vec: Vec<i32> = Vec::new();

    let num_games: usize = opponent_moves.len();
    for i in 0..num_games {
        let score_of_move = calc_score_of_move(opponent_moves[i], best_responses[i]);
